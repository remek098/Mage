    ...
    sealed class MSTransform : MSComponent<Transform>
    {
        ...
        protected override bool UpdateComponents(string propertyName)
        {
            switch (propertyName)
            {
                ...
                case nameof(PosY):
                case nameof(PosZ):
                    SelectedComponents.ForEach(comp => comp.Position = new Vector3(_posX ?? comp.Position.X, _posY ?? comp.Position.Y, _posZ ?? comp.Position.Z));
                    return true;

                case nameof(RotX):
                case nameof(RotY):
                case nameof(RotZ):
                    SelectedComponents.ForEach(comp => comp.Rotation = new Vector3(_rotX ?? comp.Rotation.X, _rotY ?? comp.Rotation.Y, _rotZ ?? comp.Rotation.Z));
                    return true;

                case nameof(ScaleX):
                case nameof(ScaleY):
                case nameof(ScaleZ):
                    SelectedComponents.ForEach(comp => comp.Scale = new Vector3(_scaleX ?? comp.Scale.X, _scaleY ?? comp.Scale.Y, _scaleZ ?? comp.Scale.Z));
                    return true;
            }
        ...
        public MSTransform(MSEntity mSEntity)
            : base(mSEntity)
        {
            Refresh();
        }
