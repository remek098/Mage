...
// maps a material's type and shader_flags to an index in the array of root signatures.
std::unordered_map<u64, id::id_type>    mat_to_rootsig_map;
...
public:
    ...
    explicit d3d12_material_stream(std::unique_ptr<u8[]>& material_buffer,
                                   material_init_info info) {
        ...
        const u32 buffer_size{
            ...
            (sizeof(id::id_type) + sizeof(u32)) * info.texture_count // texture ids and descriptor indices (may be 0 if no textures are used).
        };

        // basically reverse of what's done in initialize() function.
        ...
        *(u32*)(&buffer[texture_count_index]) = info.texture_count;
        ...
        initialize(); // read the data into member variables
...
private:
    void initialize() {
        ...
        // data ordered as specified for d3d12::content::material::add(), i.e.
        /*
        Output format:
        struct {
        ...
        */
        ...
        // if valid, we need to count how many bits are set to 1 in this u32, to know how to index into buffer. 
        // _mm_popcnt_u32 intrinsic function provides that functionality.
...
get_root_signature_flags(shader_flags::flags flags) {
    ...
    // grant access to shader stages that are required
...
create_root_signature(material_type::type type, shader_flags::flags flags) {
    ...
    auto pair = mat_to_rootsig_map.find(key);
    if (pair != mat_to_rootsig_map.end()) {
        assert(pair->first == key);
        return pair->second;
    }

    // created per material type.
    ID3D12RootSignature* root_signature{ nullptr };

    switch (type) {
        case mage::gfx::material_type::opaque:
        {
            using params = gpass::opaque_root_parameter;
            d3dx::d3d12_root_parameter parameters[params::count]{};
            parameters[params::per_frame_data].as_cbv(D3D12_SHADER_VISIBILITY_ALL, 0); // cbv 0
            ...
            parameters[params::element_buffer].as_srv(buffer_visibility, 1); // optional (normal vector, uv coords, ...)
            ...
            parameters[params::per_object_data].as_cbv(data_visibility, 1); // cbv 1
...
add(material_init_info info) {
    ...
    d3d12_material_stream{ buffer, info };
...
remove(id::id_type id) {
    std::lock_guard lock{ material_mutex };
    /*
    NOTE: We don't remove root signature because it might still be in use by another material potentially.
          We also don't remove any key-value from the unordered_map, because the keys are not unique to any material.
          Root signatures will be released only when we shutdown a renderer.
    */
    materials.remove(id);
}
