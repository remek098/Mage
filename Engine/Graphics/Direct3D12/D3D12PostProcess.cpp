...
namespace mage::gfx::d3d12::fx {
    namespace {
        ...
        // right now no need for any buffers for post-processing because all we do here right now
        // is to copy gpass's main buffer to the swapchain's backbuffer.
        ...
        bool create_fx_pso_and_root_signature() {
            ...
            // we need to read from gpass's main buffer, therefore we have to pass descriptor heap that
            // contains main buffer's SRV along with the index of that SRV within the heap.
            d3dx::d3d12_descriptor_range range{
                D3D12_DESCRIPTOR_RANGE_TYPE_SRV,
                D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND,
                0, 0, // shader register, shader space
                D3D12_DESCRIPTOR_RANGE_FLAG_DESCRIPTORS_VOLATILE // Since we're using bindless resources, 
                                                                 // we have to define an unbounded range of shader resource descriptors.
            };


            // crate FX root signature
            using idx = fx_root_param_indices;
            d3dx::d3d12_root_parameter parameters[idx::count]{};
            parameters[idx::root_constants].as_constant(1, D3D12_SHADER_VISIBILITY_PIXEL, 1); // index of main buffer's SRV within the heap
            parameters[idx::descriptor_table].as_descriptor_table(D3D12_SHADER_VISIBILITY_PIXEL, &range, 1); // GPU address of the SRV descriptor heap
            ...
            struct {
                ...
                // NOTE: no depth state nor format since post-process uses opaque geometry and doesn't have depth buffer
            } stream;
    ...
    void post_process(id3d12_graphics_command_list* cmd_list, D3D12_CPU_DESCRIPTOR_HANDLE target_rtv) {
        ...
        // NOTE: we don't need to clear the render target, because each pixel will be overwritten
        //       by pixel from gpass's main buffer.
        //       We also don't need depth buffer.
        cmd_list->OMSetRenderTargets(1, &target_rtv, 1, nullptr);
        // because in gpass::render we also draw single triangle
