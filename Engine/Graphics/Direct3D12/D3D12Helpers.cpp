...
namespace mage::gfx::d3d12::d3dx {
    ...
    ID3D12RootSignature* create_root_signature(const D3D12_ROOT_SIGNATURE_DESC1& desc) {
        ...
        if (FAILED(hr = D3D12SerializeVersionedRootSignature(&versioned_desc, &signature_blob, &error_blob))) {
            DEBUG_ONLY_EXPR(const char* error_msg = error_blob ? (const char*)error_blob->GetBufferPointer() : "");
            DEBUG_ONLY_EXPR(OutputDebugStringA(error_msg));
            return nullptr;
        }
...
ID3D12Resource* create_buffer(const void* data, u32 buffer_size,
    ...
    desc.Alignment = 0; // driver will create resource at proper alignment.
    desc.Width = buffer_size;
    desc.Height = 1; // 1d array of bytes.
    desc.DepthOrArraySize = 1;
    desc.MipLevels = 1;
    desc.Format = DXGI_FORMAT_UNKNOWN;
    desc.SampleDesc = {1, 0};
    desc.Layout = D3D12_TEXTURE_LAYOUT_ROW_MAJOR; // because buffer is a row of data ...
    desc.Flags = is_cpu_accessible ? D3D12_RESOURCE_FLAG_NONE : flags;
    
    // the buffer will be used only for upload or as constant buffer / UAV.
    ...
    if (data) {
        ...
        if (is_cpu_accessible) {
            ...
            resource->Unmap(0, nullptr); // unmapping it because we're only initializing buffer.
        }
        else { // if only accessed by gpu
