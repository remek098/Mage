...
set_up_vector(d3d12_camera camera, const void* const data, [[maybe_unused]] u32 size) {
    math::vec3 up_vector{ *(math::vec3*)data };
...
set_field_of_view(d3d12_camera camera, const void* const data, [[maybe_unused]] u32 size) {
    assert(camera.projection_type() == gfx::camera::perspective);
    f32 fov{ *(f32*)data };
...
set_view_width(d3d12_camera camera, const void* const data, [[maybe_unused]] u32 size) {
    assert(camera.projection_type() == gfx::camera::ortographic);
    f32 width{ *(f32*)data };
...
get_view(d3d12_camera camera, void* const data, [[maybe_unused]] u32 size) {
    math::mat4x4* const matrix{ (math::mat4x4* const)data };
    assert(sizeof(math::mat4x4) == size);
...
get_up_vector(d3d12_camera camera, void* const data, [[maybe_unused]] u32 size) {
    math::vec3* const up_vector{ (math::vec3* const)data };
    assert(sizeof(math::vec3) == size);
    DirectX::XMStoreFloat3(up_vector, camera.up()); // basically stores up_vector in derefferenced data pointer.
}

void
get_field_of_view(d3d12_camera camera, void* const data, [[maybe_unused]] u32 size) {
    assert(camera.projection_type() == gfx::camera::perspective);
    f32* fov{ (f32* const)data };
...
get_projection_type(d3d12_camera camera, void* const data, [[maybe_unused]] u32 size) {
    gfx::camera::type* const projection_type{ (gfx::camera::type* const)data };
    assert(sizeof(gfx::camera::type) == size);
...
void
dummy_set_fn(d3d12_camera, const void* const, u32) {}
...
d3d12_camera::d3d12_camera(camera_init_info info)
    ...
    _field_of_view{ info.field_of_view }, _aspect_ratio{ info.aspect_ratio }, // not setting other pair since those are just aliases for these
    _projection_type{ info.type }, _entity_id{ info.entity_id },
    _is_dirty{ true } {
    assert(id::is_valid(_entity_id));
    update(); // calculate all matrices.
}
...
d3d12_camera::update() {
    ...
    if (_is_dirty) {
        _projection = (_projection_type == gfx::camera::perspective)
...
void
d3d12_camera::up(math::vec3 up) {
    _up = DirectX::XMLoadFloat3(&up);
}

void
d3d12_camera::field_of_view(f32 fov) {
    assert(_projection_type == gfx::camera::perspective);
    _field_of_view = fov;
    _is_dirty = true;
}
...
d3d12_camera::view_width(f32 width) {
    assert(width);
    assert(_projection_type == gfx::camera::ortographic);
    _view_width = width;
    _is_dirty = true;
}
...
create(camera_init_info info) {
    return gfx::camera{ camera_id{cameras.add(info)} };
}
...
void
set_parameter(camera_id id, camera_parameter::parameter param, const void* const data, u32 size) {
    assert(data && size);
    assert(param < camera_parameter::count);
    d3d12_camera& camera{ get(id) };
    set_functions[param](camera, data, size);
}

void
get_parameter(camera_id id, camera_parameter::parameter param, void* const data, u32 size) {
    assert(data && size);
    assert(param < camera_parameter::count);
    d3d12_camera& camera{ get(id) };
    get_functions[param](camera, data, size);
}
