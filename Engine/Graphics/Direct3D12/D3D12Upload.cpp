...
namespace mage::gfx::d3d12::upload {
...
upload_frame                upload_frames[upload_frame_count]{}; // each thread can take any frame that is ready and use it for upload.
...
u32 get_available_upload_frame() {
    ...
    // None of the frames were done uploading. We're the only thread here, so we can iterate through frames until 
    // we find one that's ready.
    // in short -> busy wait loop.
...
d3d12_upload_context::d3d12_upload_context(u32 aligned_size) {
    ...
    // ready the upload buffer by mapping it to system memory.
    ...
    // ready up for recording commands.
...
void d3d12_upload_context::end_upload() {
    ...
    // this could be called on diffrent threads.
    // although executing command lists is thread-safe.
    ...
    // NOTE: being lazy af and using placement new operator with default constructor accessible only in debug builds
    //       to override all members.
    DEBUG_ONLY_EXPR(new (this) d3d12_upload_context{});
}
...
bool initialize() {
    ...
    for (u32 i = 0; i < upload_frame_count; ++i) {
        ...
        DXCALL(hr = device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_COPY, frame.cmd_allocator, 
                                              nullptr, IID_PPV_ARGS(&frame.cmd_list)));
        if (FAILED(hr))  return init_failed();
